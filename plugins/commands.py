...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    ...
                    InlineKeyboardButton('ꜱᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ', url="https://t.me/rajbharmvsupport"),
                    InlineKeyboardButton('ᴍᴏᴠɪᴇꜱ ɢʀᴏᴜᴘ', url='https://t.me/searchmoviesname')
                ],[
                  InlineKeyboardButton('ᴍʏ ʙᴏᴛᴢ ʟɪꜱᴛ', callback_data='source')
                ],[
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                            [
                                ...
                                InlineKeyboardButton('ʙᴏᴛ'ꜱ ꜰᴀᴛʜᴇʀ', url="https://t.me/rajbharsaheb")
                            ],
                            [
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                         [
                          ...
                          InlineKeyboardButton("ʙᴏᴛ'ꜱ ꜰᴀᴛʜᴇʀ", url="t.me/rajbharsaheb")
                         ],[ 
                            InlineKeyboardButton('🚀 ꜰᴀꜱᴛ ᴅᴏᴡɴʟᴏᴀᴅ / ᴡᴀᴛᴄʜ ᴏɴʟɪɴᴇ🖥️', callback_data=f'generate_stream_link:{file_id}') #Don't change anything without contacting me @LazyDeveloperr
                            ]
                        ]
    ...
    if not files_:
        ...
        try:
            ...
            title = '@rajbharbotz  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = '@rajbharbotz  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"@rajbharbotz  {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await check_verification(client, message.from_user.id) and VERIFY == True:
        btn = [[
            InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
        ]]
